...
func Get_buckets_bucketkey_tests_testid_stepsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_buckets_bucketkey_tests_testid_stepsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_buckets_bucketKey_tests_testId_steps",
		mcp.WithDescription("List test steps for a test."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_buckets_bucketkey_tests_testid_stepsHandler(cfg),
	}
}
