...
func Post_buckets_bucketkey_environmentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePost_buckets_bucketkey_environmentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_buckets_bucketKey_environments",
		mcp.WithDescription("Create new shared environment."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_buckets_bucketkey_environmentsHandler(cfg),
	}
}
