...
func Get_buckets_bucketkey_messages_messageidHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/buckets/%s/messages/%s", cfg.BaseURL, bucketKey, messageId)
...
func CreateGet_buckets_bucketkey_messages_messageidTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_buckets_bucketKey_messages_messageId",
		mcp.WithDescription("Retrieve the details for a single message."),
		mcp.WithString("bucketKey", mcp.Required(), mcp.Description("Unique identifier for a bucket")),
		mcp.WithString("messageId", mcp.Required(), mcp.Description("The unique identifier for this message")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_buckets_bucketkey_messages_messageidHandler(cfg),
	}
}
