...
func Delete_buckets_bucketkey_messagesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDelete_buckets_bucketkey_messagesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_buckets_bucketKey_messages",
		mcp.WithDescription("Clear a bucket (remove all messages)."),
		mcp.WithString("bucketKey", mcp.Required(), mcp.Description("Unique identifier for a bucket")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Delete_buckets_bucketkey_messagesHandler(cfg),
	}
}
