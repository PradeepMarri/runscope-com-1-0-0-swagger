...
func Get_buckets_bucketkey_errorsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/buckets/%s/errors%s", cfg.BaseURL, bucketKey, queryString)
...
func CreateGet_buckets_bucketkey_errorsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_buckets_bucketKey_errors",
		mcp.WithDescription("Retrieve a list of error messages in a bucket"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_buckets_bucketkey_errorsHandler(cfg),
	}
}
