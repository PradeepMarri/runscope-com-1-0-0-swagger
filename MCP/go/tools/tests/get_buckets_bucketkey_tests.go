...
func Get_buckets_bucketkey_testsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_buckets_bucketkey_testsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_buckets_bucketKey_tests",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_buckets_bucketkey_testsHandler(cfg),
	}
}
