...
func Delete_buckets_bucketkey_tests_testidHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDelete_buckets_bucketkey_tests_testidTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_buckets_bucketKey_tests_testId",
		mcp.WithDescription("Delete a test, including all steps, schedules, test-specific environments and results."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Delete_buckets_bucketkey_tests_testidHandler(cfg),
	}
}
