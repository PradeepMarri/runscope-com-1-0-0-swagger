...
func Post_buckets_bucketkey_tests_testid_environmentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePost_buckets_bucketkey_tests_testid_environmentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_buckets_bucketKey_tests_testId_environments",
		mcp.WithDescription("Create new test environment."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_buckets_bucketkey_tests_testid_environmentsHandler(cfg),
	}
}
