...
func Get_buckets_bucketkeyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_buckets_bucketkeyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_buckets_bucketKey",
		mcp.WithDescription("Returns a single bucket resource."),
		mcp.WithString("bucketKey", mcp.Required(), mcp.Description("Unique identifier for a bucket")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_buckets_bucketkeyHandler(cfg),
	}
}
