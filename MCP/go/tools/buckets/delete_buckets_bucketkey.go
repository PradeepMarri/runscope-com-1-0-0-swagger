...
func Delete_buckets_bucketkeyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/buckets/%s", cfg.BaseURL, bucketKey)
...
func CreateDelete_buckets_bucketkeyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_buckets_bucketKey",
		mcp.WithDescription("Delete a single bucket resource."),
		mcp.WithString("bucketKey", mcp.Required(), mcp.Description("Unique identifier for a bucket")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Delete_buckets_bucketkeyHandler(cfg),
	}
}
